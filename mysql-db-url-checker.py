...
Description: This script checks if a MySQL database credentials are valid or not.
...
def main():
    url = '' or input("Enter your MySQL database Full Connection URL or Host URL: ")
    port = ''
    user = ''
    password = ''
    database = ''
    ...
    if '@' in url:
        ...
        user = '' or input("Enter your MySQL database username: ")
        if not user:
            print(CLIColors.FAIL + "Username cannot be empty" + CLIColors.ENDC)
            sys.exit()
        password = '' or input("Enter your MySQL database password: ")
        if not password:
            print(CLIColors.FAIL + "Password cannot be empty" + CLIColors.ENDC)
            sys.exit()
        database = '' or input("Enter your MySQL database name: ")
        if not database:
            print(CLIColors.FAIL + "Database name cannot be empty" + CLIColors.ENDC)
            sys.exit()
        port = 3306 or input("Enter your MySQL database port: ")
        if not port:
        ...
        try:
            ...
            if connection.is_connected():
                print(CLIColors.OKGREEN + "Your MySQL database credentials are valid" + CLIColors.ENDC)
                sys.exit()
            else:
                print(CLIColors.FAIL + "Your MySQL database credentials are invalid" + CLIColors.ENDC)
                sys.exit()
        except Exception as e:
            print(CLIColors.FAIL + "Your MySQL database credentials are invalid" + CLIColors.ENDC)
            print("Error: " + str(e))
